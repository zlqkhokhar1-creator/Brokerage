...
interface ModernLoginProps {
  onLogin: (user: { id: string; email: string; name: string; phone: string }) => void;
...
export function ModernLogin({ onLogin, onNavigate }: ModernLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate successful login with test user data
    onLogin({
      id: 'test-user-123',
    ...
    localStorage.setItem('authToken', 'dev-mock-token-12345');
    return;
    
    // Original login code (kept for reference but not executed due to the return above)
