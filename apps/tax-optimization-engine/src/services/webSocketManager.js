...
class WebSocketManager {
  ...
  sendTaxOptimizationUpdate(userId, update) {
    try {
      const message = {
        type: 'tax_optimization_update',
        update: update,
        timestamp: new Date().toISOString()
      };
      ...
      logger.error(`Error sending tax optimization update to user ${userId}:`, error);
    }
  }

  sendTaxLossHarvestingUpdate(userId, update) {
    try {
      const message = {
        type: 'tax_loss_harvesting_update',
        update: update,
        timestamp: new Date().toISOString()
      };
      ...
      logger.error(`Error sending tax loss harvesting update to user ${userId}:`, error);
    }
  }

  sendTaxReportUpdate(userId, update) {
    try {
      const message = {
        type: 'tax_report_update',
        update: update,
        timestamp: new Date().toISOString()
      };
      ...
      logger.error(`Error sending tax report update to user ${userId}:`, error);
    }
  }
