...
import { kysely } from '../src/config/kysely';
...
class LedgerReplayVerifier {
   ...
   */
  async verifyLedger(): Promise<ReplayResult> {
    ...
    try {
      ...
      const transactionCountResult = await kysely
      ...
      for (const balance of balances) {
        try {
          const verification = await this.ledgerRepo.verifyBalanceConsistency(
            balance.entity_type,
            balance.entity_id,
            balance.currency
          );
          ...
          result.summary.errors.push(`Error verifying ${balance.entity_type}:${balance.entity_id}:${balance.currency} - ${(error as Error).message}`);
          result.status = 'FAIL';
        }
      }
      ...
      result.status = 'FAIL';
      result.summary.errors.push(`Fatal error: ${(error as Error).message}`);
   ...
   */
  async replayTransactions(dryRun: boolean = true): Promise<{
    success: boolean;
    ...
    try {
      if (!dryRun) {
        ...
        await kysely.deleteFrom('ledger_balances').execute();
      ...
      while (hasMore) {
        ...
        for (const transaction of transactions) {
          try {
            ...
            errors.push(`Error processing transaction ${transaction.id}: ${(error as Error).message}`);
          }
        }
      ...
      errors.push(`Fatal error during replay: ${(error as Error).message}`);
   ...
   */
  async generateSummaryReport(): Promise<object> {
    try {
      const summary = await kysely
        ...
        .select([
          ...
          kysely.fn.min('balance_minor').as('min_balance'),
          kysely.fn.max('balance_minor').as('max_balance')
        ])
        .groupBy(['entity_type', 'currency'])
        .execute();

      const transactionSummary = await kysely
        .selectFrom('ledger_transactions')
        .select([
          'entity_type',
          'currency',
          ...
          kysely.fn.sum('amount_minor').as('total_amount')
        ])
