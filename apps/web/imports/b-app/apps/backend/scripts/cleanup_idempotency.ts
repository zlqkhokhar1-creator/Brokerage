...
class IdempotencyCleanupService {
   ...
   */
  private async cleanupRedisKeys(redisClient: any, dryRun: boolean): Promise<number> {
      ...
      for (let i = 0; i < keys.length; i += batchSize) {
        ...
        for (const key of batch) {
          try {
            ...
            console.warn(`Error checking TTL for key ${key}:`, (error as Error).message);
          }
        }
        ...
        if (keysToRemove.length > 0 && !dryRun) {
          try {
            ...
            console.warn(`Error deleting Redis keys:`, (error as Error).message);
          }
        } else if (dryRun) {
          totalRemoved += keysToRemove.length;
        }
        ...
        if (keysToRemove.length > 0) {
          console.log(`Processed batch ${Math.floor(i / batchSize) + 1}: ${keysToRemove.length} expired keys`);
        }
      }
  ...
  async getCleanupStats(): Promise<{
    ...
    try {
      ...
      const expiredResult = await kysely
        .selectFrom('idempotency_keys')
        .select(kysely.fn.count('id').as('count'))
        .where('expires_at', '<', new Date())
        .executeTakeFirstOrThrow();

      const completedResult = await kysely
        .selectFrom('idempotency_keys')
        .select(kysely.fn.count('id').as('count'))
        .where('completed_at', 'is not', null)
        .executeTakeFirstOrThrow();

      const pendingResult = await kysely
        .selectFrom('idempotency_keys')
        .select(kysely.fn.count('id').as('count'))
   ...
   */
  async cleanupOlderThan(days: number, dryRun: boolean = false): Promise<CleanupResult> {
